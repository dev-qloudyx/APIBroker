...
class CaseViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        headers = self.get_success_headers(serializer.data)
        owner = self.request.user.id
        files_list = self.request.FILES.getlist('case_file')
        attachment_list = self.request.FILES.getlist('attachment')
        check_attachment = validate_attachment(attachment_list) # Validate file attachment
        
        if (files_list and check_serializer and check_attachment[1]):
            ...
            return Response({'resultCode': 0, 'errorDescription': {'Parameters':serializer.errors, 'Allowed ContentTypes': f"{check_attachment[2]}", 'Attachments': f'{check_attachment[0]}'}}, status=status.HTTP_400_BAD_REQUEST, headers=headers)
        kwargs.pop('case_files') # Pop because self.request.FILES is not serializable
        save_to_db.delay(**kwargs) # Create assync task and pass serializer.data as kwargs
        return Response({"Response": 1}, status=status.HTTP_201_CREATED, headers=None)
    ...
    def retrieve(self, request, pk=None, case_number=None, plate_number=None, customer_key=None, skey=None):
        if (pk):
            try:
                case = Case.objects.filter(id=pk, owner=request.user)
            except ValueError as e:
                return Response({"Response": 0, 'errorDescription': f'{e}'})
        if (case_number):
        ...
        if (skey):
            case = Case.objects.filter(
               preshared_key=skey, owner=request.user)
        serializer = CaseSerializer(instance=case, many=True, context={'request': request})
        if (serializer.data):
            return Response({"Response": 1, "Case": serializer.data[0]}, status=status.HTTP_200_OK)
        else:
            return Response({"Response": 0, 'errorDescription': 'Not found'}, status=status.HTTP_400_BAD_REQUEST)
